...
@WebServlet(value = "/cwe-306/BenchmarkTestCWE306_01")
public class BenchmarkTestCWE306_01 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String username = request.getParameter("BenchmarkTestCWE306_01_user");
        String password = request.getParameter("BenchmarkTestCWE306_01_pass");

        boolean validCredentials =
                "admin".equals(username) && "password123".equals(password);

        // CWE-306: a crafted request parameter is accepted as authentication evidence.
        boolean parameterBypass =
                "administrator".equals(request.getParameter("access"));

        if (validCredentials || parameterBypass) {
            writeAdministratorPage(response, parameterBypass ? "request parameter" : "credentials");
        } else {
